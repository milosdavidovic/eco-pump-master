...
namespace TestBedPro
{
        class tehKar
        {
            ...
            public Xaxis _CurveSetX
            {
                get { return GetCurveSetXJSON(_referenca); }
                set { _CurveSetX = value; }
            }

            public Yaxis _CurveSetY
            {
                get { return GetCurveSetYJSON(_referenca); }
                set { _CurveSetY = value; }
            }
            ...
            public Xaxis GetCurveSetXJSON(string referenca)

            {
                ...
                Uri url = new Uri("http://product-selection.grundfos.com/product-detail.pumpcurve.specification.json?frequency=50&languagecode=SRL&unitsystem=4&productnumber=" + referenca);
                try
                {
                    string data = client.DownloadString(url);
                    RootObject1 j = JsonConvert.DeserializeObject<RootObject1>(data);
                    xBre = j.xaxis;
            ...
            public Yaxis GetCurveSetYJSON(string referenca)
            {
                Yaxis yBre = null;
                // otvaramo vezu
                WebClient client = new WebClient();
